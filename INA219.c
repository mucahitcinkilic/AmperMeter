...
void wireWriteRegister (uint8_t reg, uint16_t value)
{
	uint8_t i2c_temp[2];
	i2c_temp[0] = value>>8;
	i2c_temp[1] = value & 0xFF;  // Alt byte (LSB)
	HI2C_WriteA(I2C_2, INA219_ADDRESS<<1, reg, i2c_temp, 2);
	DelayMs(1);
}
...
{
	uint8_t i2c_temp[2];
 // Register adresini yaz ve ardından 16-bitlik veriyi oku
    if (HI2C_ReadA(I2C_2, INA219_ADDRESS<<1, reg, i2c_temp, 2)) 
    {
        // MSB ve LSB'yi birleştirerek 16-bit değeri geri döndür
